...
def names(tid, q, brand, model, side):
    ...
    ar = join([ty['ar'], qd['ar'] if qd else '', b['ar'], model, SIDES[side]['ar'] if side else ''])
    en_core = join([b['en'], model, ty['en'], qd['en'] if qd else ''])
...
def descriptions(name, brand, model, side, sku):
    ...
    he = (f"{name['he']} – חלק חילוף חדש (תחליפי, לא מקורי){' ל' + v_he if v_he else ''}. {s_he}"
    ...
    ar = (f"{name['ar']} – قطعة غيار جديدة (بديلة، غير أصلية){' لشاحنات ' + v_ar if v_ar else ''}. {s_ar}"
    ...
    en = (f"{name['en']} – new aftermarket replacement part{' for ' + v_en + ' trucks' if v_en else ''}. {s_en}"
...
def specs(tid, brand, model, side):
    ...
    if model: row('דגם', 'الطراز', 'Model', model, model, model)
...
for r in rows:
    ...
    sku = p['sku'] or r['sku']
    d_he, d_ar, d_en, short_he = descriptions(name, brand, model, side, sku)
    ...
    payload.append({
        ...
        'compat': [vehicle(brand, model, 'en')] if model else [],
    })
...
print(f"with model: {sum(1 for x in payload if x['compat'])} | without model: {sum(1 for x in payload if not x['compat'])}")
