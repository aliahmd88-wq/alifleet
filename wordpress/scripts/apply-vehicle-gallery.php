...
function alifleet_to_webp( string $src ): ?string {
	...
	$im = 'png' === $ext ? @imagecreatefrompng( $src ) : @imagecreatefromjpeg( $src );
	if ( ! $im ) return null;
	imagepalettetotruecolor( $im ); imagealphablending( $im, false ); imagesavealpha( $im, true );
	$out = preg_replace( '/\.(png|jpe?g)$/i', '.webp', $src ); imagewebp( $im, $out, 82 ); imagedestroy( $im );
	return $out;
}
$done = 0;
foreach ( $map as $slug => $m ) {
	...
	$first = preg_replace( '/\.(png|jpe?g)$/i', '.webp', $m['files'][0] );
